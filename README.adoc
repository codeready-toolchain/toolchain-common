...
In a new terminal, login as admin and execute the following commands from the local `host-operator` repository directory
```
...
then execute the following commands from the local `member-operator` repository directory
```
$ make create-namespace 
$ make deploy-rbac
$ make deploy-crd
...
Assumption: This script assumes that you have host and member operators running on Openshift cluster
